...
import type { FoundOperation } from "@/server/base/swagger/types.js";
...
export async function handleSwaggerGetModelTool(request: CallToolRequest) {
  ...
  let fragmentTag: string | undefined;
  let fragmentOperationId: string | undefined;
  ...
  if (fragmentOperationId && !fragmentTag) {
    const found = findOperationById(doc as any, fragmentOperationId);
    if (found) {
    ...
    // 回退：尝试当作 tag 匹配
    const tagOps = findOperationsByTag(doc as any, fragmentOperationId);
    if (tagOps.length > 0) {
      const list = tagOps.map((op: FoundOperation) => {
        const io = extractOperationIO(doc as any, op);
        return {
  ...
  if (fragmentTag) {
    ...
    const operations = findOperationsByTag(doc as any, fragmentTag);
    if (operations.length > 0) {
      const list = operations.map((op: FoundOperation) => {
        const io = extractOperationIO(doc as any, op);
        return {
          path: io.operation.path,
          method: io.operation.method,
