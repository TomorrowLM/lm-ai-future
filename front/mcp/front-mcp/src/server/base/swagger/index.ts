...
export const swaggerGetModelTool = {
  ...
  description:
    "查询 Swagger/OpenAPI：支持完整单接口 URL、Tag URL、显式 operationId/tag、兼容 name 模型查询，以及分页模型目录；请求体和响应体可解析 $ref。",
...
async function searchAcrossGroups(
  ...
  try {
    resources = (await tryFetchJson(
      swaggerResourcesUrl,
      SWAGGER_NETWORK_TIMEOUT_MS.resources,
    )) as unknown[];
  } catch {
    return undefined;
  }
  ...
  for (const resource of resources) {
    const item = resource as Record<string, unknown> | undefined;
    ...
    try {
      ...
      if (!isValidSpec(doc)) continue;
      const found = findOperationByKeyword(doc as Record<string, unknown>, apiPath);
...
export async function handleSwaggerGetModelTool(request: CallToolRequest) {
  ...
  const modelNames = Object.keys(getSchemasRoot(doc)).sort((left, right) =>
