 ...
 */
export function findOperationsByTag(doc: any, tag: string): FoundOperation[] {
  const paths = doc?.paths;
  ...
  for (const [pathKey, item] of Object.entries(paths)) {
    ...
    for (const [method, operation] of Object.entries(item as any)) {
      ...
      if (tags.some((t: string) => String(t).toLowerCase() === targetTag)) {
 ...
 */
export function findOperationById(doc: any, operationId: string): FoundOperation | undefined {
  const paths = doc?.paths;
  if (!paths || typeof paths !== "object") return undefined;
