...
import { buildCreateUiInstruction } from "./instruction.js";
const currentFilePath = fileURLToPath(import.meta.url);
const currentDir = path.dirname(currentFilePath);
// 解析输入的页面配置路径，支持绝对路径和相对于当前工作目录或当前文件目录的相对路径
async function resolvePageFilePath(inputPath) {
...
function getInvalidApiRequests(page) {
    return (page.requests ?? [])
        .map((request, index) => ({ request, index }))
        .filter(({ request }) => !request.apiName?.trim());
}
// 验证页面定义的完整性和正确性
function validatePageDefinition(page) {
    if (!page.name) {
    ...
    if (invalidApiRequests.length > 0) {
        const details = invalidApiRequests
            ...
            .join("；");
        throw new Error(`create_ui_mcp: page.requests 中存在缺失 apiName 的项：${details}。请先执行 create_api_mcp，并将返回的 apiName 回填到 page.json 后再执行 create_ui_mcp。`);
    }
}
...
export async function handleCreateUiTool(request) {
    ...
    try {
        ...
        throw new Error(`create_ui_mcp: 页面配置 JSON 解析失败 ${pageFilePath}: ${err instanceof Error ? err.message : String(err)}`);
    }
    if (!payload.page) {
