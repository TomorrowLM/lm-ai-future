...
function getToolRequirements(tool) {
    return (tool.requirements ?? []).map((item) => item.trim()).filter(Boolean);
}
function buildDependencyRequirementLines(depends) {
    if (!depends) {
        return [];
    }
    const lines = [];
...
function buildRequirementLines(requirements) {
...
async function buildChildTask(child) {
    const requirements = [
...
export async function buildCreateUiInstruction(page) {
