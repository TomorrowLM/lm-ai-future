 ...
 */
export function resolveSchemaNode(options) {
    ...
    if (Array.isArray(node.allOf)) {
        const parts = node.allOf.map((part) => resolveSchemaNode({ doc, node: part, depth: depth - 1, seenRefs }));
        return { ...node, allOf: parts };
    }
    if (Array.isArray(node.oneOf)) {
        const parts = node.oneOf.map((part) => resolveSchemaNode({ doc, node: part, depth: depth - 1, seenRefs }));
        return { ...node, oneOf: parts };
    }
    if (Array.isArray(node.anyOf)) {
        const parts = node.anyOf.map((part) => resolveSchemaNode({ doc, node: part, depth: depth - 1, seenRefs }));
        return { ...node, anyOf: parts };
    }
    const resolved = Array.isArray(node) ? [] : { ...node };
