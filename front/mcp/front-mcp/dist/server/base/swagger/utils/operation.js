 ...
 * 通过关键词查找操作
 */
export function findOperationByKeyword(doc, keyword) {
    ...
    const scoreOf = (op, method, pathKey) => {
        const textParts = [
            op?.summary,
            op?.description,
            op?.operationId,
        ...
        if (!haystack.includes(needle))
            return -1;
        let score = 1;
        if (String(op?.summary ?? "").toLowerCase().includes(needle))
            score += 3;
        if (String(op?.operationId ?? "").toLowerCase().includes(needle))
            score += 2;
        if (String(pathKey).toLowerCase().includes(needle))
            score += 1;
        return score;
    };
    let best;
 ...
 */
export function pickFirstContentSchema(content) {
 ...
 * 提取操作的输入输出
 */
export function extractOperationIO(doc, found) {
    ...
    const bodyParam = parameters.find((p) => p && p.in === "body" && p.schema);
