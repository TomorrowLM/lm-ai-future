...
6. **同一业务领域的枚举、颜色映射、选项配置必须集中在同一文件中**，不拆分到多个文件。例：`src/constants/event.ts` 同时包含 `EventStatus` 枚举、`EVENT_STATUS_COLOR_MAP` 颜色映射和 `EVENT_OVERDUE_OPTIONS` 选项
7. **跨模块复用的枚举必须放在 `src/constants/` 对应文件中**（如 `event.ts`），不放在页面级 `types.ts`；仅页面私有的枚举才放页面目录的 `types.ts`
8. **禁止在业务代码中使用状态/等级/类型等魔数**，必须引用枚举常量。例：用 `EventStatus.Pending` 代替 `10`，用 `EventLevel.Urgent` 代替 `10`

#### 文件命名规范

- 主组件文件：`index.tsx`
- 样式文件：`index.module.less`（仅在 Tailwind 无法覆盖时使用，必须使用 CSS Modules）
- 类型定义：`types.ts`
- 常量定义：`consts.ts`
- 工具函数：`utils.ts`
- 测试文件：`index.test.tsx`
